...
#model_datetime = datetime.datetime(2016, 11, 30, 6, 0)                      # Starting time for WRF modeling (hour, minute)
...
#    def get_q(event_datetime, number_of_time_points = 1, name):
       
# get quantity of "name" microphysical fracture from 2m to 20 kilometers height
...
def el_field_q_int_z_t(event_datetime, name, number_of_time_points, time_point_of_start  = 1):
...
def create_time_array_in_seconds(start_hour, event_datetime, number_of_time_points, step, time_point_of_start  = 1):
# step is the quantity of hours in one "time unit"
# if "time unit" = 5 min, then step = 1/12
    
    time_index = get_index(event_datetime, time_point_of_start )
    time_array_in_seconds = [start_hour] * number_of_time_points
    
    for j in range(0, number_of_time_points):
        time_array_in_seconds[j] += (time_index + j) * step
    return time_array_in_seconds
...
def main():
    ...
    start_hour = 12;
    #event_datetime = datetime.datetime(2016, 4, 26, 12, 0)
    ...
    # plt.plot(get_t2(event_datetime, number_of_time_points = 36))
...
#    plt.axis('image')
    ...
    number_of_time_points = 288;
    step = 1/12;
    plt.plot(create_time_array_in_seconds(start_hour, event_datetime, number_of_time_points, 1/12, time_point_of_start  = 1), np.array(el_field_q_int_z_t(event_datetime, name, number_of_time_points)))
